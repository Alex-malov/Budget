...
test("создаёт и обновляет факт часов подрядчика из оперативной таблицы", async function() {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "planning-contractor-hours-"));
  const snapshotPath = path.join(directory, "snapshot.json");
  const subcontractPath = path.join(directory, "subcontracts.json");
  const referencePath = path.join(directory, "references.json");
  const teamPath = path.join(directory, "team.json");
  ...
  await fs.writeFile(snapshotPath, JSON.stringify({
    finance: { years: [2026], lines: [], operating: {} },
    reference: { roles: ["Роль"], projects: ["Проект"] }, projects: ["Проект"], cashReceipts: [], staffResources: [], subcontracts: [],
    team: { roles: [], roster: [], resourcePlan: [{ id: "contractor-plan", employee: "Ресурс подрядчика", project: "Проект", role: "Роль", source: "Подряд", hoursPlan: { 2026: { 1: 160 } } }] }
  }), "utf8");
  const server = createServer(snapshotPath, subcontractPath, referencePath, teamPath, staffPath, changeLogPath);
  await new Promise(function(resolve) { server.listen(0, "127.0.0.1", resolve); });
  const baseUrl = "http://127.0.0.1:" + server.address().port;
  ...
  const postReference = function(directoryName, body) {
    return request("/api/references/" + directoryName, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  };
  try {
    assert.equal((await postReference("vendors", { name: "Поставщик", providerType: "Подряд" })).status, 201);
    const references = await request("/api/references");
    const resource = references.body.directories.resources.records.find(function(item) { return item.name === "Ресурс подрядчика"; });
    assert.ok(resource);
    const linkedResource = await request("/api/references/resources/" + encodeURIComponent(resource.id), { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: resource.name, vendor: "Поставщик", costPlan: resource.costPlan }) });
    assert.equal(linkedResource.status, 200);
    const team = await request("/api/team");
    const plan = team.body.records.find(function(item) { return item.employee === "Ресурс подрядчика" && item.project === "Проект"; });
    assert.ok(plan);

    const created = await request("/api/subcontracts/actual-hours", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ teamId: plan.id, year: 2026, month: 1, actualHours: 120 }) });
    assert.equal(created.status, 200);
    assert.equal(created.body.action, "created");
    assert.equal(created.body.record.actualHours, 120);

    const updated = await request("/api/subcontracts/actual-hours", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ teamId: plan.id, year: 2026, month: 1, actualHours: 140.5 }) });
    assert.equal(updated.status, 200);
    assert.equal(updated.body.action, "updated");
    assert.equal(updated.body.record.actualHours, 140.5);

    const records = await request("/api/subcontracts");
    assert.equal(records.body.records.filter(function(item) { return item.resource === "Ресурс подрядчика" && item.period === "2026-01"; }).length, 1);
  } finally {
    await new Promise(function(resolve) { server.close(resolve); });
    await fs.rm(directory, { recursive: true, force: true });
  }
