(function () {
  ...
  const state = {
    snapshot: null, overview: null, subcontracts: [], otherSubcontractRecords: [], staffRecords: [], teamRecords: [], references: {}, financial: { incomes: [], payments: [] }, activeTab: 0,
    ...
    subcontractHoursView: "registry", staffHoursView: "registry", teamHoursView: "registry",
    ...
    financeFilters: {
      planfact: { years: [], projects: null },
      income: { years: [], projects: null }
    },
    financeContractorMode: "payments", financeOtherMode: "work"
  };
  ...
  const subtitles = [
    "План-факт финансового результата, денежных потоков и распределения.",
    "Поступления — независимые события Плана и Факта с НДС.",
    ...
    "Оплаты подрядчикам и сверка с начислениями ресурсного подряда."
  ];
  ...
  function applyHoursContextDefaults() {
    if (state.activeTab === 2 && state.project === "all") {
      const firstProject = referenceRecords("projects")[0];
      if (firstProject) state.project = firstProject.name;
    }
  ...
  function updateContextControlsVisibility() {
    ...
    const financialOnly = state.activeTab === 0 || state.activeTab === 1;
    filterPanel.hidden = nsiOnly;
    projectFilter.closest("label").hidden = nsiOnly || financialOnly;
    yearFilter.closest("label").hidden = nsiOnly || financialOnly;
  ...
  function renderContextTabFilters() {
    ...
    if ((state.activeTab === 0 || state.activeTab === 1) && financialFeature) {
      const scope = state.activeTab === 1 ? "income" : "planfact";
      contextTabFilters.innerHTML = financialFeature.contextMarkup(scope);
      financialFeature.bindContext(scope);
      return;
    }
  ...
  function renderPipeline() {
    if (financialFeature) return financialFeature.renderPlanFact();
  ...
  function renderIncome() {
    if (financialFeature) return financialFeature.renderIncome();
  ...
  function costStatus(plan, fact) {
    ...
    if (!plan && fact) return { label: "Факт без плана", tone: "neutral" };
    if (fact > plan) return { label: "Выше плана", tone: "neutral" };
    if (fact < plan) return { label: "Ниже плана", tone: "neutral" };
  ...
  function projectCostTable(model) {
    ...
    const body = rows.map(function(row) {
      ...
      return '<tr class="cost-row level-' + row.level + '"><td><span class="cost-hierarchy level-' + row.level + '">' + (row.level === 1 ? "▾" : (row.level === 2 ? "↳" : "Σ")) + '</span><strong>' + escapeHtml(row.name) + '</strong>' + details + '</td>' + projectCostCell(row.plan, row.fact, "base") + projectCostCell(row.plan, row.fact, "resourceCost") + projectCostCell(row.plan, row.fact, "attraction") + projectCostCell(row.plan, row.fact, "vat") + projectCostCell(row.plan, row.fact, "total") + '</tr>';
    }).join("") || '<tr><td colspan="6">' + empty("Нет строк для выбранных локальных фильтров.") + '</td></tr>';
    return '<div class="table-wrap project-cost-table-wrap"><table class="project-cost-table"><thead><tr><th>Источник / объект</th><th>Расходы без НДС</th><th>Себестоимость ресурсов</th><th>Привлечение ресурсов</th><th>НДС</th><th>Расходы с НДС</th></tr></thead><tbody>' + body + '</tbody></table></div>';
  }
  ...
  function bindOtherSubcontractControls() {
    if (financialFeature) financialFeature.bindOtherControls();
  ...
  function hoursViewToggle(scope, value, compactLabel) {
    const registryActive = value === "registry";
    return '<div class="view-toggle hours-view-toggle" role="group" aria-label="Представление таблицы часов">' +
      '<button type="button" data-hours-view="' + escapeHtml(scope) + ':registry" class="' + (registryActive ? "active" : "") + '" aria-pressed="' + registryActive + '">Реестр</button>' +
      '<button type="button" data-hours-view="' + escapeHtml(scope) + ':hours" class="' + (!registryActive ? "active" : "") + '" aria-pressed="' + (!registryActive) + '">' + escapeHtml(compactLabel) + '</button>' +
      '</div>';
  }
  ...
  function renderSubcontracts() {
    ...
    const editorYear = currentHoursYear();
    const editorRows = subcontractPlanRows([editorYear]).filter(function(row) {
      return row.teamRecords.length && subcontractHasHours(row, [editorYear], allMonths());
    });
    const planTable = state.subcontractHoursView === "hours"
      ? subcontractActualHoursEditor(editorRows, editorYear)
      : (allYears ? subcontractAllYearsTable(page.rows, years, months) : subcontractTable(page.rows, years[0], months));
    ...
    const editorNote = 'Оперативное представление показывает фактические часы каждого подрядного ресурса по месяцам ' + editorYear + ' года. Дважды щёлкните по ячейке часов, чтобы изменить значение.';
    return '<section class="metric-grid compact hours-metric-grid">' +
      ...
      '</section>' +
      '<section class="panel subcontract-plan-table">' + sectionTitle("Суммы и часы подряд", state.subcontractHoursView === "hours" ? editorNote : (allYears ? "Годовые итоги часов с возможностью раскрытия месяцев." : "Часы указаны в разрезе месяцев выбранного года."), state.subcontractHoursView === "hours" ? editorYear : (allYears ? "все годы" : years[0])) +
      '<div class="table-toolbar"><span class="table-edit-hint">' + (state.subcontractHoursView === "hours" ? "Факт часов редактируется непосредственно в таблице." : "Строки формируются по НСИ и плану команды; двойной клик по доступному полю — редактирование") + '</span>' + hoursViewToggle("subcontract", state.subcontractHoursView, "Факт по месяцам") + (state.subcontractHoursView === "registry" ? subcontractPager(page, displayedRows.length) : "") + '</div>' +
      planTable +
      '<p class="table-note">' + (state.subcontractHoursView === "hours" ? "При первом вводе факта для ресурса и месяца приложение создаёт связанную запись автоматически." : ((allYears && state.subcontractViewMonth === "all" ? "Нажмите на название года, чтобы развернуть его по месяцам." : "Показаны строки, у которых есть план или факт часов в выбранном контексте.") + selectedMonthNote + " Стоимость, ставка и привлечение поступают из НСИ «Сотрудник / ресурс»; плановые часы — из раздела 06. Годовые показатели рассчитываются по плановым часам отображаемого периода. Факт и затраты присоединяются по проекту и ресурсу." + (subcontractSelectedPeriod() ? "" : " Для изменения конкретной записи выберите один год и месяц в контексте просмотра."))) + '</p></section>';
  }
  ...
  function updateProjectFilterOptions() {
    ...
    projectFilter.innerHTML = '<option value="all">Все проекты</option>' + projects.map(function(project) {
      const item = referenceRecords("projects", true).find(function(record) { return record.name === project; });
      const label = item && item.code ? item.code + " — " + item.name : project;
      return '<option value="' + escapeHtml(project) + '">' + escapeHtml(label) + '</option>';
    }).join("");
  ...
  async function refreshReferenceData() {
    ...
    state.financial = financial || { incomes: [], payments: [] };
    buildPlanIndexes();
  ...
  function inlineRecord(cell) {
    ...
    if (scope === "subcontractPlan") return findTeamRecord(cell.dataset.inlineId);
  ...
  function inlineEditorDefinition(cell, record) {
    ...
    if (scope === "reference") {
      ...
      return { type: "text", value: record[field] == null ? "" : record[field] };
    }
    if (scope === "team") {
    ...
    if (scope === "subcontractPlan") {
      const year = cell.dataset.inlineYear;
      const month = cell.dataset.inlineMonth;
      const value = state.subcontracts.filter(function(item) {
        return !item.archived && sameText(item.resource, record.employee) && sameText(item.project, record.project) && item.period === String(year) + "-" + String(month).padStart(2, "0");
      }).reduce(function(total, item) { return total + num(item.actualHours); }, 0);
      return { type: "number", value: value, min: "0", step: "0.5" };
    }
  ...
  async function saveInlineEdit(cell, record, definition, value, originalHtml) {
    ...
    if (scope === "reference") {
      ...
      body = { name: record.name, code: record.code, providerType: record.providerType, vendor: record.vendor, category: record.category, costPlan: record.costPlan, vatPlan: record.vatPlan, financialKind: record.financialKind, project: record.project, year: record.year, rate: record.rate, archived: record.archived };
      body[field] = value;
    } else if (scope === "team") {
    ...
    } else if (scope === "subcontractPlan") {
      endpoint = "/api/subcontracts/actual-hours";
      body = {
        teamId: record.id,
        year: cell.dataset.inlineYear,
        month: cell.dataset.inlineMonth,
  ...
  function showReferenceModal(directory, record) {
    ...
    const fields = vendor
      ...
      : (otherSubcontract
          ...
          : (project
            ? '<label>Код <b>*</b><input name="code" value="' + escapeHtml(item.code || "") + '" pattern="[A-Za-z0-9_-]{2,32}" ' + (editing ? "" : "autofocus") + '><small data-error="code"></small></label><label>Наименование <b>*</b><input name="name" value="' + escapeHtml(item.name) + '" ' + (editing ? "autofocus" : "") + '><small data-error="name"></small></label>'
  ...
  async function restoreReference(directory, record) {
    try {
      const response = await fetch("/api/references/" + directory + "/" + encodeURIComponent(record.id), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: record.name, code: record.code, value: record.value, providerType: record.providerType, vendor: record.vendor, category: record.category, costPlan: record.costPlan, vatPlan: record.vatPlan, financialKind: record.financialKind, project: record.project, year: record.year, rate: record.rate, archived: false })
      });
      const payload = await response.json();
  ...
  function bindSubcontractControls() {
    if (financialFeature) financialFeature.bindContractorControls();
  ...
  function bindHoursViewControls() {
    document.querySelectorAll("[data-hours-view]").forEach(function(button) {
      button.addEventListener("click", function() {
        const parts = String(button.dataset.hoursView || "").split(":");
        const scope = parts[0];
        const view = parts[1] === "hours" ? "hours" : "registry";
        if (scope === "subcontract") {
          state.subcontractHoursView = view;
          if (view === "hours") {
            state.year = currentHoursYear();
            state.subcontractViewMonth = "all";
          }
        } else if (scope === "staff") {
          state.staffHoursView = view;
          if (view === "hours") {
            state.year = currentHoursYear();
            state.staffMonth = "all";
          }
        } else if (scope === "team") {
          state.teamHoursView = view;
          if (view === "hours") state.year = currentHoursYear();
        }
        render();
      });
    });
  ...
  function renderStaff() {
    ...
    const editorYear = currentHoursYear();
    const editorRecords = staffRecordsForContext([editorYear], allMonths(), false);
    const planTable = state.staffHoursView === "hours"
      ? staffActualHoursEditor(editorRecords, editorYear)
      : (allYears ? staffAllYearsTable(records, years, months, false) : staffTable(records, years[0], months, false));
    ...
    const editorNote = 'Оперативное представление показывает фактические часы штатных сотрудников по месяцам ' + editorYear + ' года. Дважды щёлкните по ячейке часов, чтобы изменить значение.';
    return '<section class="metric-grid compact hours-metric-grid">' +
      ...
      '</section>' +
      '<section class="panel staff-plan-table">' + sectionTitle("Суммы и часы штат", state.staffHoursView === "hours" ? editorNote : (allYears ? "Годовые итоги плановых и фактических часов с возможностью раскрытия месяцев." : "План и факт часов в разрезе месяцев выбранного года."), state.staffHoursView === "hours" ? editorYear : (allYears ? "все годы" : years[0])) +
      '<div class="table-toolbar"><span class="table-edit-hint">' + (state.staffHoursView === "hours" ? "Факт часов редактируется непосредственно в таблице." : "Строки формируются по НСИ и плану команды; двойной клик по доступному полю — редактирование") + '</span>' + hoursViewToggle("staff", state.staffHoursView, "Факт по месяцам") + '</div>' +
      planTable +
  ...
  function subcontractActualHoursEditorCell(row, year, month) {
    const teamRecord = row.teamRecords && row.teamRecords[0];
    return inlineCell(integer(subcontractActualHours(row, year, month)), teamRecord ? {
      scope: "subcontractPlan", id: teamRecord.id, field: "actualHours", year: year, month: month
    } : null, "hours-cell actual-hours");
  }

  function subcontractActualHoursEditor(rows, year) {
    const months = allMonths();
    const body = rows.length ? rows.map(function(row) {
      const actualCells = months.map(function(month) { return subcontractActualHoursEditorCell(row, year, month); }).join("");
      const total = subcontractHoursTotal(row, [year], months, subcontractActualHours);
      return '<tr class="' + inactiveResourceRowClass(row.resource) + '"><td><strong>' + escapeHtml(row.resource) + '</strong></td><td>' + escapeHtml(row.resourceVendor || "—") + '</td><td>' + escapeHtml(row.project) + '</td><td>' + escapeHtml(row.role || "—") + '</td>' + actualCells + '<td class="hours-cell total-hours">' + integer(total) + '</td></tr>';
    }).join("") : '<tr><td colspan="17">' + empty("Нет подрядных ресурсов для текущего года.") + '</td></tr>';
    return '<div class="table-wrap"><table class="hours-editor-table subcontract-actual-hours-editor"><thead><tr><th>Сотрудник</th><th>Поставщик</th><th>Проект</th><th>Роль</th>' + months.map(function(month) { return '<th>' + teamMonthLabels[month - 1] + '</th>'; }).join("") + '<th>Итого</th></tr></thead><tbody>' + body + '</tbody></table></div>';
  }
  ...
  function staffActualHoursEditor(records, year) {
    const months = allMonths();
    const body = records.length ? records.map(function(record) {
      const actualCells = months.map(function(month) {
        return inlineCell(integer(staffActualHours(record, year, month)), {
          scope: "staff", id: record.id, field: "hoursActual", year: year, month: month
        }, "hours-cell actual-hours");
      }).join("");
      const teamRecord = record.teamRecord || staffTeamRecord(record);
      return '<tr class="' + inactiveResourceRowClass(record.employee) + '"><td><strong>' + escapeHtml(record.employee) + '</strong></td><td>' + escapeHtml(staffVendorForTeamRecord(teamRecord)) + '</td><td>' + escapeHtml(record.project) + '</td><td>' + escapeHtml(record.role) + '</td>' + actualCells + '<td class="hours-cell total-hours">' + integer(staffHoursTotal(record, [year], staffActualHours, months)) + '</td></tr>';
    }).join("") : '<tr><td colspan="17">' + empty("Нет штатных сотрудников для текущего года.") + '</td></tr>';
    return '<div class="table-wrap"><table class="hours-editor-table staff-actual-hours-editor"><thead><tr><th>Сотрудник</th><th>Поставщик</th><th>Проект</th><th>Роль</th>' + months.map(function(month) { return '<th>' + teamMonthLabels[month - 1] + '</th>'; }).join("") + '<th>Итого</th></tr></thead><tbody>' + body + '</tbody></table></div>';
  }
  ...
  function teamPlanHoursEditor(records, year) {
    const months = allMonths();
    const body = records.length ? records.map(function(record) {
      const planCells = months.map(function(month) { return teamHoursCell(record, year, month, false); }).join("");
      return '<tr class="' + inactiveResourceRowClass(record.employee) + '"><td><strong>' + escapeHtml(record.employee) + '</strong></td><td>' + escapeHtml(record.vendor || "—") + '</td><td>' + escapeHtml(record.project) + '</td><td>' + escapeHtml(record.role) + '</td>' + planCells + '<td class="hours-cell total-hours">' + integer(teamYearTotal(record, year)) + '</td></tr>';
    }).join("") : '<tr><td colspan="17">' + empty("Нет записей команды с планом часов на текущий год.") + '</td></tr>';
    return '<div class="table-wrap"><table class="hours-editor-table team-plan-hours-editor"><thead><tr><th>Сотрудник / ресурс</th><th>Поставщик</th><th>Проект</th><th>Роль</th>' + months.map(function(month) { return '<th>' + teamMonthLabels[month - 1] + '</th>'; }).join("") + '<th>Итого</th></tr></thead><tbody>' + body + '</tbody></table></div>';
  }
  ...
  function renderTeam() {
    ...
    const editorYear = currentHoursYear();
    const editorRecords = active.filter(function(record) { return teamHasPlan(record, [editorYear]); });
    const planTable = state.teamHoursView === "hours"
      ? teamPlanHoursEditor(editorRecords, editorYear)
      : (allYears ? teamAllYearsTable(active, years, false) : teamTable(headers, active, year, false));
    ...
    const editorNote = 'Оперативное представление показывает плановые часы по сотрудникам и проектам за ' + editorYear + ' год. Дважды щёлкните по ячейке часов, чтобы изменить значение.';
    return '<section class="metric-grid compact">' +
      ...
      '<section class="panel team-plan-table">' + sectionTitle("Команда и план часов", state.teamHoursView === "hours" ? editorNote : (allYears ? "Годовые итоги с возможностью раскрытия месяцев." : "Часы указаны в разрезе месяцев выбранного года."), state.teamHoursView === "hours" ? editorYear : (allYears ? "все годы" : year)) +
      '<div class="table-toolbar"><span class="table-edit-hint">' + (state.teamHoursView === "hours" ? "Плановые часы редактируются непосредственно в строке проекта." : "Строки формируются по НСИ; двойной клик по доступному полю — редактирование. По умолчанию проекты свёрнуты в сотрудника.") + '</span>' + hoursViewToggle("team", state.teamHoursView, "План по месяцам") + '</div>' +
      planTable +
      (archived.length ? '<details class="archive-details"><summary>Архивные записи · ' + integer(archived.length) + '</summary>' + archiveTable + '</details>' : "") +
      '</section>' +
  ...
  function referenceDirectory(directory) {
    ...
    const otherSubcontract = directory === "otherSubcontracts";
    const project = directory === "projects";
    const headers = vendor ? ["Наименование", "Тип поставщика", "Статус", ""] : (resource ? ["Сотрудник / ресурс", "Поставщик", "Тип поставщика", "Статус", ""] : (otherSubcontract ? ["Статья/Подрядчик", "Тип поставщика", "Категория", "НДС", "Статус", ""] : (project ? ["Код", "Наименование", "Статус", ""] : (providerType ? ["Тип поставщика", "Статус", ""] : ["Наименование", "Статус", ""]))));
    const row = function(record, archivedRow) {
      ...
      const cells = (project ? inlineCell('<strong>' + escapeHtml(record.code || "Код не назначен") + '</strong>', editConfig("code")) : "") + inlineCell('<strong>' + escapeHtml(record.name) + '</strong>', editConfig("name")) +
  ...
  function renderReference() {
    ...
    return '<section class="nsi-catalog-layout" aria-label="Нормативно-справочная информация">' +
      ...
      '<div class="nsi-catalog-workspace">' + (directory === "financialRates" && financialFeature ? financialFeature.renderRatesReference() : referenceDirectory(directory)) + '</div>' +
      '</section>';
  }

  financialFeature = window.BudgetFinancial && window.BudgetFinancial.create({
    state: state,
    ...
    teamHours: teamHours,
    teamCostValues: teamCostValues,
    costContractorActualHours: costContractorActualHours,
    costStaffActualHours: costStaffActualHours,
    vendorVatRate: vendorVatRate,
    otherSubcontractAmounts: otherSubcontractAmounts,
    otherSubcontractReference: otherSubcontractReference,
    ...
    render: render,
    currentYear: currentHoursYear
  });

  function renderContractorCalculations() {
    return financialFeature
      ? financialFeature.renderContractorCalculations()
      : empty("Расчёты подряд временно недоступны.");
  }

  const renderers = [renderPipeline, renderIncome, renderCosts, renderSubcontracts, renderStaff, renderTeam, renderReference, renderOtherSubcontracts, renderContractorCalculations];

  function renderNavigation() {
    const items = state.snapshot.tabs.map(function(tab, index) { return { tab: index === 0 ? "Финансовый план-факт" : tab, index: index, order: index >= 3 ? index + 3 : index + 1 }; });
    items.splice(3, 0, { tab: "Прочий подряд", index: 7, order: 4 }, { tab: "Расчёты подряд", index: 8, order: 5 });
  ...
  function render() {
    ...
    pageTitle.textContent = isReferencePage ? "Нормативно-справочная информация" : (state.activeTab === 0 ? "Финансовый план-факт" : (state.activeTab === 7 ? "Прочий подряд" : (state.activeTab === 8 ? "Расчёты подряд" : state.snapshot.tabs[state.activeTab])));
    ...
    if (state.activeTab === 8 && financialFeature) financialFeature.bindContractorControls();
    if (state.activeTab === 2) bindProjectCostControls();
    if ((state.activeTab === 0 || state.activeTab === 1) && financialFeature) financialFeature.bindFinancialControls();
    if (state.activeTab === 6 && state.referenceDirectory === "financialRates" && financialFeature) financialFeature.bindRatesControls();
    bindInlineEditing();
    bindHoursViewControls();
  ...
  function populateFilters() {
    ...
    document.getElementById("reset-filters").addEventListener("click", function() {
      ...
      state.subcontractHoursView = "registry";
      state.staffHoursView = "registry";
      state.teamHoursView = "registry";
